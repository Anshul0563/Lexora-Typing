...
export function calculateResult(source, typed, elapsedSeconds, telemetry = {}, scoringRule = {}) {
  ...
  const accuracy = comparison.referenceCharacters
    ? Math.max(0, (comparison.referenceCharacters - comparison.weightedErrors) / comparison.referenceCharacters * 100)
    : (comparison.typedCharacters ? 0 : 100);
  ...
  return {
    ...
    ...characterProjection(comparison),
