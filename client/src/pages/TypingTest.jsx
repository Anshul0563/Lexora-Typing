...
const selectableTestModes = [
  { value: 'TCS', name: 'TCS Mode', description: 'Primary SSC typing mode for TCS iON-style exams', title: 'Use for SSC CGL, CHSL, Stenographer, MTS and applicable Railway typing tests.' },
  { value: 'Standard', name: 'Standard', description: 'Uses this exam\'s custom scoring rule', title: 'Use only when an administrator has configured a non-TCS practice rule for this exam.' },
  { value: 'NTA', name: 'NTA Mode', description: 'Reserved for future NTA-based CBT support', title: 'NTA exams such as JEE Main, NEET UG, CUET and UGC NET are generally MCQ-based CBTs, not SSC typing tests.', disabled: true }
];
...
export default function TypingTest() {
  ...
  const chooseMode = (mode) => { if (selectableTestModes.find((item) => item.value === mode)?.disabled) return; selectedModeRef.current = mode; setSelectedMode(mode); };

  const persistSession = useCallback((overrides = {}) => {
    if (!data) return;
    sessionStorage.setItem(storageKey, JSON.stringify({ data, typed: typedRef.current, endAt: endAtRef.current, testToken: testTokenRef.current, totalKeystrokes: keystrokesRef.current, backspaceCount: backspacesRef.current, selectedMode: selectedModeRef.current, backspaceEnabled, wordHighlight, ...overrides }));
    ...
    if (saved) {
      try {
        ...
        if (session.data?.exam?._id === examId && session.endAt && session.testToken) {
          const restoredMode = testModes.includes(session.selectedMode) ? session.selectedMode : 'TCS';
  ...
  const begin = async () => {
    ...
    try {
      const session = await api(`/exams/${examId}/start`, { method: 'POST', body: JSON.stringify({ paragraphId: data.paragraph._id, testMode: selectedModeRef.current }) });
      const remaining = Math.max(0, session.endsAt - Date.now()); testTokenRef.current = session.testToken; endAtRef.current = session.endsAt; monotonicEndRef.current = performance.now() + remaining; activeRef.current = true; setSeconds(Math.ceil(remaining / 1000)); setPhase('active'); persistSession({ selectedMode: selectedModeRef.current, endAt: session.endsAt, testToken: session.testToken });
  ...
  if (phase === 'mode-select') return <main className="instruction-page"><Brand /><section className="instruction-card instruction-card-wide"><h1>Select Test Mode</h1><p className="lead">TCS Mode is recommended for SSC typing practice.</p><div className="mode-selector"><div className="mode-options">{selectableTestModes.map((mode) => <button key={mode.value} type="button" className={`mode-button ${selectedMode === mode.value ? 'active' : ''}`} onClick={() => chooseMode(mode.value)} disabled={mode.disabled} title={mode.title} aria-describedby={`${mode.value.toLowerCase()}-mode-desc`}><span className="mode-name">{mode.name}</span><span id={`${mode.value.toLowerCase()}-mode-desc`} className="mode-desc">{mode.description}</span>{mode.disabled && <span className="mode-status">Disabled</span>}</button>)}</div></div><div className="mode-info"><p><strong>Use TCS for SSC.</strong> SSC CGL, CHSL, Stenographer, MTS and many applicable Railway typing exams use a TCS iON-style interface. NTA Mode is not for SSC typing and is reserved for future NTA CBT support.</p></div><Button className="button-large" onClick={() => setPhase('instructions')}>Continue with {selectedMode}</Button><button className="plain-button" onClick={() => navigate('/dashboard')}>Cancel</button></section></main>;
