...
export default function TypingTest() {
  ...
  const chooseMode = (mode) => { selectedModeRef.current = mode; setSelectedMode(mode); };
    ...
    if (saved) {
      try {
        ...
        if (session.data?.exam?._id === examId && session.endAt && session.testToken) {
          const restoredMode = testModes.includes(session.selectedMode) ? session.selectedMode : 'Standard';
  ...
  if (phase === 'mode-select') return <main className="instruction-page"><Brand /><section className="instruction-card instruction-card-wide"><h1>Select Test Mode</h1><p className="lead">Choose how you want to take this test</p><div className="mode-selector"><div className="mode-options"><button className={`mode-button ${selectedMode === 'Standard' ? 'active' : ''}`} onClick={() => chooseMode('Standard')}><span className="mode-name">Standard</span><span className="mode-desc">Uses this exam's configured scoring rule</span></button><button className={`mode-button ${selectedMode === 'TCS' ? 'active' : ''}`} onClick={() => chooseMode('TCS')}><span className="mode-name">TCS Mode</span><span className="mode-desc">Net WPM uses character errors divided by 5</span></button><button className={`mode-button ${selectedMode === 'NTA' ? 'active' : ''}`} onClick={() => chooseMode('NTA')}><span className="mode-name">NTA Mode</span><span className="mode-desc">Net WPM uses uncorrected word errors</span></button></div></div><div className="mode-info"><p><strong>What's the difference?</strong> Your selected mode is locked into the test session and applied during backend scoring.</p></div><Button className="button-large" onClick={() => setPhase('instructions')}>Continue with {selectedMode}</Button><button className="plain-button" onClick={() => navigate('/dashboard')}>Cancel</button></section></main>;
